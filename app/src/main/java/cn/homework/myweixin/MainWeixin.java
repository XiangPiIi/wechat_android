...
public class MainWeixin extends Activity {
	...
	String UserId;
	private RecyclerView recyclerView; // 声明为成员变量
...
//        this.UserId=getIntent().getStringExtra("USER_ID");
		...
		// 初始化第一个分页的RecyclerView
		View view1 = LayoutInflater.from(this).inflate(R.layout.main_tab_weixin, null);
		recyclerView = view1.findViewById(R.id.recyclerView);
		recyclerView.setLayoutManager(new LinearLayoutManager(this));
		// 创建适配器并设置数据
		List<FriendEntity> friendChatList = new ArrayList<>();

		adapter = new FriendChatAdapter(friendChatList);
		recyclerView.setAdapter(adapter);
		// Fetch friend chats from the database
		fetchFriendChats(adapter);
...
//        View view1 = mLi.inflate(R.layout.main_tab_weixin, null);
    ...
    //查询好友列表
	private void fetchFriendChats(FriendChatAdapter adapter) {
		OkHttpClient client = new OkHttpClient();
		String url = "http://10.0.2.2:9090/api/friends?userId=" + UserId; // 替换为实际的API地址
		...
		client.newCall(request).enqueue(new Callback() {
			...
			public void onResponse(Call call, Response response) throws IOException {
				...
				if (response.isSuccessful()) {
					...
					List<FriendEntity> friendChatList = parseFriendChats(jsonData);

					runOnUiThread(() -> {
						//更新查询到的好友列表
						adapter.updateData(friendChatList);
	...
	private List<FriendEntity> parseFriendChats(String jsonData) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				friend.setSenderId(UserId); // 使用传入的userId作为发送者
				friend.setFriendId(jsonObject.getString("friendId"));
				// 解析Timestamp
				if(jsonObject.isNull("lastMessageTime"))
					friend.setLastTime(null);
				else
				{
					Long Lasttime = jsonObject.getLong("lastMessageTime");
					friend.setLastTime(new Date(Lasttime));
				}

				friend.setAvatarUrl(jsonObject.getString("avatarUrl"));
				friend.setLastMessage(jsonObject.getString("lastMessage"));
				friend.setFriendName(jsonObject.getString("friendName"));
				friendList.add(friend);
			}
			Log.d("FriendListSize", "Parsed friends count: " + friendList.size());
		} catch (JSONException e) {
			e.printStackTrace();
		} catch (IllegalArgumentException e) {
			e.printStackTrace(); // 处理Timestamp解析异常
		}
		return friendList;
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_CODE && resultCode == RESULT_OK) {
			if (data != null && data.getBooleanExtra("refresh", false)) {
				fetchFriendChats(adapter); // 调用刷新方法
			}
		}
	}
